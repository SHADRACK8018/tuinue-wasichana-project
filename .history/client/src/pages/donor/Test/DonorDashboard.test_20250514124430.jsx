...
describe("DonorDashboard", () => {
  ...
  test("adds and removes charity from favorites", async () => {
    ...
    // Find the favorite button inside the first charity card by class name, wait for it to appear
    const firstCard = charityCards[0];
    const favButton = await waitFor(() =>
      firstCard.querySelector("button.favorite-button")
    );
    ...
    const removeFavButton = await waitFor(() =>
      favCard.querySelector("button.favorite-button")
    );
