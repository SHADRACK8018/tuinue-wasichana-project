...
describe("BeneficiaryStory Component", () => {
  ...
  it("opens and closes modal with full story", async () => {
    ...
    // Check modal is closed (modal dialog no longer visible)
    await waitFor(() => {
  ...
  it("toggles see more and see less in modal", async () => {
    ...
    // Try to find "See More" button, if not found, find "See Less"
    let toggleButton;
    try {
      toggleButton = within(screen.getByRole("dialog")).getByText(/see more/i);
      ...
      toggleButton = within(screen.getByRole("dialog")).getByText(/see less/i);
      fireEvent.click(toggleButton);

      // Check text has changed to "See More"
      expect(
        within(screen.getByRole("dialog")).getByText(/see more/i)
      ).toBeInTheDocument();

      // Click "See More" and check it changes back
      fireEvent.click(
        within(screen.getByRole("dialog")).getByText(/see more/i)
      );
      expect(
