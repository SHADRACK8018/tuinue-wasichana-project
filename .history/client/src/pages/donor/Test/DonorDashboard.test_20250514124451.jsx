...
describe("DonorDashboard", () => {
  ...
  test("adds and removes charity from favorites", async () => {
    ...
    // Find the favorite button inside the first charity card by aria-label, wait for it to appear
    const firstCard = charityCards[0];
    const favButton = await within(firstCard).findByRole("button", {
    ...
    const removeFavButton = await within(favCard).findByRole("button", {
      name: /add to favorites|remove from favorites/i,
    });
